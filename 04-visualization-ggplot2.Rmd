...
ggplot(data = specimens_complete, aes(x = weight, y = length)) +
    geom_point(alpha = 0.1, aes(color=scientificName))
```

> ### Challenge
>
> Use what you just learned to create a scatter plot of `weight` over `scientificName` with the plot types showing in different colors.

...
source("setup.R")
specimens_complete <- read.csv(file = "data_output/specimens_complete.csv")
```
...
> sets with large numbers of observations, such as the `specimens_complete` data
...
> ### Challenge
>
> Use what you just learned to create a scatter plot of `weight` over `scientificName` with the genera showing in different colors.
...
>
> - Add color to the datapoints on your boxplot according to the month from which
>   the specimen was taken (`month`).
...
the aesthetic function to include `group = scientificName`:
...
yearly_weight <- specimens_complete %>%
                 group_by(year, scientificName) %>%

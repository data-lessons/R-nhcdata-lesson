...
The first few rows of the output are full of `NA`s, if we wanted to remove those
...
>  `hindfoot_half` containing values that are half the `length` values.
...
```{r, eval=FALSE, purl=FALSE}
## Answer
specimens_hindfoot_half <- specimens %>%
    filter(!is.na(length)) %>%
    mutate(hindfoot_half = length / 2) %>%
    filter(hindfoot_half < 30) %>%
    select(scientificName, hindfoot_half)
```
--->
...
with the mean body weight of each genera by institution.

...
frame (`specimens`), and the subsequent arguments are the columns to keep.
...
In the above, we use the pipe to send the `specimens` dataset first through
`filter()` to keep rows where `weight` is less than 50, then through `select()`
to keep only the `scientificName`, `sex`, and `weight` columns. Since `%>%` takes
...
> ### Challenge {.challenge}
>
>  Using pipes, subset the `specimens` data to include individuals collected before
...
Remember that some of our weights are `NA`s, if we wanted to remove those
we could insert a `filter()` in the chain:
...
>  criteria: contains only the `scientificName` column and a new column called
>  `weight_half` containing values that are half the `weight` values.
>  In this `weight_half` column, there are no `NA`s and all values are less
>  than 30.
>
...
```{r, eval=FALSE, purl=FALSE}
## Answer
specimens_weight_half <- specimens %>%
    filter(!is.na(weight)) %>%
    mutate(weight_half = weight / 2) %>%
    filter(weight_half < 30) %>%
...
When grouping both by `sex` and `scientificName`, the first rows are for individuals
that have not had a sex determined. 

We can also remove the missing values for weight before we
...
`tibble`. If you want to display more data, you can use the `print()` function
...
specimens %>%
    ...
    select(year, genus, scientificName, weight) %>%
    arrange(year)
...
**wide** data format. For the `specimens` data as we have it right now, this is
going to be one heck of a wide data frame! However, if we were to summarize data
within states and genus, we might begin to have some relationships we'd want to
examine.

Let's see this in action. First, using **`dplyr`**, let's create a data frame
with the mean body weight of each genera by state.
...
don't have any record in that state. Sometimes it is fine to leave those as
...
go backwards from `specimens_gw_wide`, and exclude `stateProvince` from the gathering,
...
> ### Challenge {.challenge}
>
> 1. Make a wide data frame with `year` as columns, `stateProvince` as rows, and the
>   values are the number of genera per state. You will need to summarize before
...
>    `stateProvince` `year` combination.
>
> 3. The `specimens` data set is not truly wide or long because there are
>    two columns of measurement - `length` and `weight`.  This makes it
...
>    `measurement` in each `year` for each different `stateProvince`. Then
>    `spread` them into a wide data set with a column for `length` and
...
## Answer 2
rich_time %>%
  gather(year, n_genera, -stateProvince)

## Answer 3
specimens_long <- specimens %>%
  gather(measurement, value, length, weight)

## Answer 4
specimens_long %>%
  group_by(year, measurement, stateProvince) %>%
...
Let's start by removing observations for which the `scientificName` is missing. In
...
`length` are missing. This dataset should also only contain
...
`specimens_complete` has `r nrow(specimens_complete)` rows and `r ncol(specimens_complete)`
columns by typing `dim(specimens_complete)`.

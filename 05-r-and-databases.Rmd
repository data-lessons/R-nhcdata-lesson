...
source("setup.R")
if (file.exists("idigbio_rodents.sqlite")) file.remove("idigbio_rodents.sqlite")
```
...
The SQLite database is contained in a single file `portal_rodents.sqlite`
...
the `SQLite` database contained in the `portal_rodents.sqlite` file.
